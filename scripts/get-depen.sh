...
if [ $RESOURCE_NAME == 'pods' ];then
    kube-lineage  $RESOURCE_NAME  $NAME  --dependencies  -n $NAMESPACE  --context=$CONTEXT 
    ...
    echo "$labels" | while IFS= read -r line; do
        ...
        kube-lineage  svc $SVC  --output=split --show-group -n $NAMESPACE  --context=$CONTEXT 
    done
    ...
    kube-lineage  $RESOURCE_NAME  $NAME  --output=split --show-group -n $NAMESPACE  --context=$CONTEXT
elif [[ $RESOURCE_NAME == 'clusterrolebindings' || $RESOURCE_NAME == 'clusterroles' ]];then
    kube-lineage  $RESOURCE_NAME  $NAME  --output=split --show-group  --context=$CONTEXT
else
    kube-lineage  $RESOURCE_NAME  $NAME  --dependencies  -n $NAMESPACE  --context=$CONTEXT 
fi
